...
class Earth(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X: Any, y: Any, sample_weight: Any | None = None) -> Earth:
        ...
        self.record_ = EarthRecord(X_processed, y_processed, self)
        ...
        use_rust_fit = (
            ...
            and getattr(ForwardPasser.run, "__module__", "pymars._forward")
            == "pymars._forward"
            and PruningPasser.__module__ == "pymars._pruning"
            and getattr(PruningPasser.run, "__module__", "pymars._pruning")
            == "pymars._pruning"
        )
        rust_fitted = (
    ...
    def summary(self) -> str | None:
        ...
        if self.basis_ and self.coef_ is not None:
            ...
            for i, bf in enumerate(self.basis_):
                ...
                if isinstance(coef_val, np.ndarray):
