...
class ForwardPasser:
    ...
    def run(
        ...
        if self.y_train.ndim > 1 and self.y_train.shape[1] > 1:
            raise ValueError(
                "ForwardPasser currently supports only single-output targets (y should be 1D)."
            )
        ...
        self.current_B_matrix = self._build_basis_matrix(
            self.X_train, self.current_basis_functions
        )
        ...
        if self.model.record_ is not None and hasattr(
        ...
        while True:
            ...
            if (
                self._best_candidate_addition is None
                or self._min_candidate_rss >= self.current_rss - EPSILON
            ):
                break
            ...
            if (
                len(self.current_basis_functions) + num_terms_to_add_this_step
                > max_terms_for_loop
            ):
                break

            gcv_before, _ = self._calculate_gcv_for_basis_set(
                self.current_basis_functions
            )
            ...
            if bf2_cand_or_None is not None:
                bf2_cand_or_None.gcv_score_ = (
                    gcv_reduction if gcv_reduction is not None else 0.0
                )
    ...
    def _calculate_gcv_for_basis_set(
        ...
        if (
            num_actual_coeffs == 0
        ):  # Should be caught by coeffs is None, but as a safeguard
            return np.inf, None
        ...
        if (
            B_matrix.size > 0
            and num_terms_in_b_matrix > 0
            and num_actual_coeffs == 0
            and num_valid_rows > 0
        ):
    ...
    def _generate_candidates(
        ...
        for parent_bf in self.current_basis_functions:
            ...
            for var_idx in range(self.n_features):
                ...
                if (
                    ...
                    or var_idx not in self.categorical_features
                ):
                    if self.model.allow_linear:
                        ...
                        if not exists_linear:
                            linear_candidate = LinearBasisFunction(
        ...
        if self.model.allow_missing and self.missing_mask is not None:
            ...
            for var_idx in range(self.n_features):
                if (
                    var_idx in current_missingness_vars
                ):  # Already have an is_missing term for this variable
                    continue
                ...
                if np.any(
                    ...
                    if (
                        self.model.record_
                        and hasattr(self.model.record_, "feature_names_in_")
                        and self.model.record_.feature_names_in_ is not None
                        and var_idx < len(self.model.record_.feature_names_in_)
                    ):
                    ...
                    missing_bf_candidate = MissingnessBasisFunction(
                        variable_idx=var_idx, variable_name=var_name
                    )
    ...
    def _find_best_candidate_addition(self):
        ...
        for bf1, bf2_or_None in candidate_additions:
            ...
            gcv_candidate = calculate_gcv(
                rss_candidate, num_valid_rows_candidate, eff_params
            )

            if gcv_candidate < self._min_candidate_gcv - EPSILON or (
