...
class BasisFunction(ABC):
    ...
    def _set_properties(
        ...
        self.parent2 = (
            parent2  # Not currently used for 2nd order interactions via parent2
        )
...
class HingeBasisFunction(BasisFunction):
    ...
    def transform(
        ...
        if X_processed.ndim == 2 and self.variable_idx >= X_processed.shape[1]:
            raise IndexError(
                f"variable_idx {self.variable_idx} is out of bounds for X_processed with {X_processed.shape[1]} features."
            )
