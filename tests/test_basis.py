...
def test_constant_basis_function():
    ...
    dummy_missing_mask_1d = np.zeros_like(
        X_1d, dtype=bool
    )  # For 1D X, mask should be (N,) or (N,1)
    if (
        X_1d.ndim == 1
        and dummy_missing_mask_1d.ndim == 2
        and dummy_missing_mask_1d.shape[1] == 1
    ):
...
def test_hinge_basis_function(is_right, parent_type):
    ...
    if parent_type == "Constant":
        ...
        parent_bf = HingeBasisFunction(
            variable_idx=1, knot_val=1.5, is_right_hinge=True, variable_name="X1"
        )
        expected_parent_str = f"({str(parent_bf)}) * "
        parent_degree = 1
    ...
    bf = HingeBasisFunction(
        variable_idx=base_var_idx,
        knot_val=base_knot,
        is_right_hinge=is_right,
    ...
    expected_hinge_str = (
        f"max(0, {base_var_name} - {base_knot:.2f})"
        if is_right
        else f"max(0, {base_knot:.2f} - {base_var_name})"
    )
...
def test_linear_basis_function(parent_type):
    ...
    bf = LinearBasisFunction(
        variable_idx=base_var_idx, variable_name=base_var_name, parent_bf=parent_bf
    )
...
def test_missingness_basis_function():
    ...
    missing_mask_some = np.array(
        [
            ...
            [
                False,
                True,
            ],  # FeatureB missing for sample 1 (should not affect bf for FeatureA)
    ...
    transformed_var1 = bf_var1.transform(
        X_dummy, missing_mask_some
    )  # Using mask where var_idx=1 has a missing value
    ...
    with pytest.raises(
        IndexError,
        match="variable_idx 0 is out of bounds for missing_mask with 0 features",
    ):
        bf.transform(
            X_dummy, np.empty((X_dummy.shape[0], 0), dtype=bool)
        )  # Mask with 0 features

    bf_high_idx = MissingnessBasisFunction(variable_idx=5, variable_name="FeatureF")
    with pytest.raises(
        IndexError,
        match="variable_idx 5 is out of bounds for missing_mask with 2 features",
    ):
