...
def initial_model_for_pruning(simple_pruning_data):
    ...
    bf_hinge_x1_k13 = HingeBasisFunction(
        variable_idx=1, knot_val=13.0, is_right_hinge=False, variable_name="x1_k13L"
    )
    bf_hinge_x1_k16 = HingeBasisFunction(
        variable_idx=1, knot_val=16.0, is_right_hinge=True, variable_name="x1_k16R"
    )
    bf_linear_x2 = LinearBasisFunction(variable_idx=2, variable_name="x2")
    bf_extra_x0_k5 = HingeBasisFunction(
        variable_idx=0, knot_val=5.0, is_right_hinge=True, variable_name="x0_extra_k5R"
    )
    bf_noise_x1_k11 = HingeBasisFunction(
        variable_idx=1,
        knot_val=11.0,
        is_right_hinge=True,
        variable_name="x1_noise_k11R",
    )
    ...
    B_initial = temp_passer._build_basis_matrix(
        X, initial_bfs, temp_passer.missing_mask
    )
    _, initial_coeffs, _ = temp_passer._calculate_rss_and_coeffs(B_initial, y.ravel())

    if initial_coeffs is None:
        pytest.skip(
            "Could not calculate initial coefficients for pruning test setup due to LSTSQ failure."
        )
...
def test_pruning_run_no_pruning_if_penalty_zero(initial_model_for_pruning):
    ...
    pruned_bfs, pruned_coeffs, best_gcv = passer.run(
        ...
        X_fit_original=X,
        initial_basis_functions=initial_bfs_copy,
        initial_coefficients=initial_coeffs,
    )

    passer_check = PruningPasser(earth_model)
    passer_check.X_train = X
    passer_check.y_train = y.ravel()
    passer_check.n_samples = X.shape[0]
    passer_check.missing_mask = dummy_missing_mask
    passer_check.X_fit_original = X
...
def test_pruning_run_some_pruning_expected(initial_model_for_pruning):
    ...
    pruned_bfs, pruned_coeffs, best_gcv = passer.run(
        ...
        missing_mask=dummy_missing_mask,
        X_fit_original=X,
        initial_basis_functions=list(initial_bfs),
        initial_coefficients=initial_coeffs,
    )
...
def test_pruning_run_intercept_protection(simple_pruning_data):
    ...
    bf_truly_bad_hinge = HingeBasisFunction(
        variable_idx=2, knot_val=1000, is_right_hinge=True, variable_name="bad_hinge_x2"
    )
    ...
    if initial_coeffs is None:
        pytest.skip(
            "Initial coeffs calculation failed for intercept protection test setup."
        )
