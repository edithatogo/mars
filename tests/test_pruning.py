...
def test_compute_gcv_for_subset(simple_pruning_data, initial_model_for_pruning):
    ...
    gcv1, rss1, coeffs1 = passer._compute_gcv_for_subset(X, y.ravel(), X.shape[0], initial_bfs)
    ...
    gcv2, rss2, coeffs2 = passer._compute_gcv_for_subset(X, y.ravel(), X.shape[0], subset_bfs)
    ...
    gcv_intercept, rss_intercept, coeffs_intercept = passer._compute_gcv_for_subset(X, y.ravel(), X.shape[0], [intercept_bf])
...
def test_pruning_run_no_pruning_if_penalty_zero(initial_model_for_pruning):
    ...
    gcv_of_pruned_set, _, _ = passer_check._compute_gcv_for_subset(X, y.ravel(), X.shape[0], pruned_bfs)
    assert np.isclose(best_gcv, gcv_of_pruned_set), "Returned best_gcv should match GCV of returned pruned_bfs"

    gcv_of_initial_set, _, _ = passer_check._compute_gcv_for_subset(X, y.ravel(), X.shape[0], initial_bfs) # Use original full set
...
def test_pruning_run_some_pruning_expected(initial_model_for_pruning):
    ...
    gcv_initial_pen3, _, _ = passer_check._compute_gcv_for_subset(X, y.ravel(), X.shape[0], initial_bfs)
...
def test_pruning_run_intercept_protection(simple_pruning_data):
    ...
    passer_check = PruningPasser(earth_model)
    # Must set X_train, y_train, n_samples on passer_check if its _compute_gcv_for_subset relies on them
    # However, we modified _compute_gcv_for_subset to take them as args.
    gcv_intercept_only, _, _ = passer_check._compute_gcv_for_subset(
        X, y.ravel(), X.shape[0], [bf_intercept] # Pass X, y, n_samples directly
    )
