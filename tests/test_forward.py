...
class MockEarth(Earth):
    def __init__(self, max_degree=1, max_terms=10,
                 ...
                 minspan=-1, endspan=-1,
                 penalty=3.0, allow_linear=True, allow_missing=False): # Added allow_missing
...
def test_initial_model_setup_in_run(simple_data):
    ...
    bfs, coeffs = passer.run(X_fit_processed=X, y_fit=y.ravel(), missing_mask=dummy_missing_mask, X_fit_original=X)
...
def test_calculate_rss_and_coeffs(simple_data):
    ...
    rss_const, coeffs_const, num_valid_const = passer._calculate_rss_and_coeffs(B_const, y)
    ...
    rss_singular, coeffs_singular, num_valid_sing = passer._calculate_rss_and_coeffs(B_singular, y)
...
def test_find_best_addition_simple_original(simple_data):
    ...
    rss, coeffs, _ = passer._calculate_rss_and_coeffs(passer.current_B_matrix, passer.y_train)
...
def test_get_allowable_knot_values(simple_data):
    ...
    knots_inter_ms = passer_inter_minspan._get_allowable_knot_values(X[:,0], parent_hinge_interaction, 0)
    ...
    passer_inactive_parent.n_samples, passer_inactive_parent.n_features = X_active_test.shape
...
def test_run_main_loop_simple_case(simple_data):
    X, y = simple_data
    earth_model = MockEarth(max_degree=1, max_terms=3, endspan_alpha=0.0, allow_linear=True)
    passer = ForwardPasser(earth_model)
    dummy_missing_mask = np.zeros_like(X, dtype=bool)
    final_bfs, final_coeffs = passer.run(X_fit_processed=X, y_fit=y.ravel(), missing_mask=dummy_missing_mask, X_fit_original=X)
    ...
    final_bfs_mt1, final_coeffs_mt1 = passer_mt1.run(X_fit_processed=X, y_fit=y.ravel(), missing_mask=dummy_missing_mask_mt1, X_fit_original=X)
    ...
    final_bfs_mt2, final_coeffs_mt2 = passer_mt2.run(X_fit_processed=X, y_fit=y.ravel(), missing_mask=dummy_missing_mask_mt2, X_fit_original=X)
    assert len(final_bfs_mt2) == 2

def test_generate_candidates_for_interaction(interaction_data):
    X, y = interaction_data
    earth_model = MockEarth(max_degree=2, max_terms=5, endspan_alpha=0.0, allow_linear=True)
    passer = ForwardPasser(earth_model)
    passer.X_train = X
    passer.y_train = y.ravel()
    passer.n_samples, passer.n_features = X.shape
    ...
    bf_hinge_x0 = HingeBasisFunction(variable_idx=0, knot_val=1.5, is_right_hinge=True, variable_name="x0", parent_bf=bf_intercept)
...
def test_run_with_interaction(interaction_data):
    X, y = interaction_data
    earth_model = MockEarth(max_degree=2, max_terms=7, penalty=0, endspan_alpha=0.0, allow_linear=True)
    passer = ForwardPasser(earth_model)

    dummy_missing_mask_inter = np.zeros_like(X, dtype=bool)
    final_bfs, final_coeffs = passer.run(X_fit_processed=X, y_fit=y.ravel(), missing_mask=dummy_missing_mask_inter, X_fit_original=X)
...
def test_find_best_addition_selects_linear(simple_data):
    ...
    passer.current_basis_functions = [intercept_bf]
    passer.current_B_matrix = passer._build_basis_matrix(passer.X_train, passer.current_basis_functions)
    rss_intercept, coeffs_intercept, _ = passer._calculate_rss_and_coeffs(passer.current_B_matrix, passer.y_train)
...
def test_run_adds_linear_term():
    ...
    earth_model = MockEarth(max_degree=1, allow_linear=True, max_terms=2,
...
def test_run_adds_linear_interaction_term():
    ...
    earth_model = MockEarth(max_degree=2, allow_linear=True, max_terms=5,
                           endspan_alpha=0.0, minspan_alpha=0.0, penalty=0)
    passer = ForwardPasser(earth_model)
