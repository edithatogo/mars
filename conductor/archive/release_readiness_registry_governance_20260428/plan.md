...
The automated registry ownership checks in this lane are complete. The current
review-based gates are tracked separately: R `marsearth` is confirmed and
queued in CRAN `incoming/newbies/` pending screening, while Julia `MarsEarth`
needs a new General registration because `MarsRuntime` is a separate published
package identity.
...
- [x] Task: Audit public package-name availability
    ...
    - [x] Confirm R `marsearth` and Julia `MarsEarth` remain release targets; `MarsRuntime` is superseded
- [x] Task: Confirm registry ownership and credential status [R marsearth submitted; Julia MarsEarth needs new General registration]
    ...
    - [x] Confirm R r-universe/CRAN requirements and Julia General registration path [R marsearth confirmed in CRAN incoming/newbies pending screening; Julia MarsEarth needs new registration]
...
- [x] Task: Conductor - Automated Review and Checkpoint 'Phase 0: Registry and Ownership Inventory' (Protocol in workflow.md)
