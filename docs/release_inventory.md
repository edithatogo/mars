...
| npm `mars-earth` | Maintainer-owned package name; publish token or trusted publishing path still needs to be finalized for OTP-free publishing | Confirm npm org and automation-token/trusted-publishing path | Owner confirmed | 2026-05-03 |
...
- Rust, npm, and NuGet publish jobs already reference the expected secret names
  or trusted-publishing paths in GitHub Actions.
