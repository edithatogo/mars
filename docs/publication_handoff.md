...
| R `marsearth` | Monitor CRAN incoming/screening and check-result pages; supersede the earlier `marsruntime` / `mars.earth` uploads | Release checklist, `bindings/r/cran-comments.md`, and maintainer notes | CRAN maintainer confirmation complete; queued in `incoming/newbies/` pending screening |
| Julia `MarsEarth` | Register new Julia package; keep `MarsRuntime` as superseded legacy package | Release checklist and maintainer notes | Needs new Julia General registration; no registration PR yet |

## How to Provide It

- Put secret values into GitHub Actions secret settings, not in the repository.
- For R, monitor the renamed `marsearth` CRAN entry in `incoming/newbies/`,
  watch for screening/pretest email, and treat the earlier `marsruntime` /
  `mars.earth` CRAN uploads as superseded. For Julia, register `MarsEarth` as a
  new package and keep `MarsRuntime` as superseded legacy.
