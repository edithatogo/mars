...
and usage. The mars library addresses this limitation by providing a
...
The primary contribution of this paper and the mars library is to make
...
The mars library extends beyond the core MARS algorithm with several
...
the MARS algorithm, Section 3 describes the mars implementation and
...
Compared to existing MARS implementations, mars offers several
distinct advantages:
...
\item
  ...
  C/Cython extensions, mars is pure Python, simplifying installation
  and deployment
\item
...
The mars implementation follows a modular architecture designed for
...
\tightlist
\item
  \texttt{mars/earth.py}: The main \texttt{Earth} class implementing
  the MARS algorithm with scikit-learn compatibility
\item
  \texttt{mars/\_sklearn\_compat.py}: Provides \texttt{EarthRegressor}
  and \texttt{EarthClassifier} wrappers for scikit-learn compatibility
\item
  \texttt{mars/\_forward.py}: Implements the forward pass algorithm
  for basis function selection
\item
  \texttt{mars/\_pruning.py}: Implements the backward pass algorithm
  for model pruning
\item
  \texttt{mars/\_basis.py}: Defines various basis function types
  (hinge, linear, categorical, missingness)
\item
  \texttt{mars/\_util.py}: Utility functions for GCV calculation and
  other common operations
\item
  \texttt{mars/glm.py}: Generalized linear model extensions for
...
\item
  \texttt{mars/explain.py}: Advanced interpretability features
...
The basis function system in mars is designed with flexibility and
...
mars provides robust support for missing values and categorical
...
The mars library provides full compatibility with the scikit-learn
ecosystem:
...
The \texttt{GLMEarth} class extends mars to support generalized linear
...
The following example demonstrates the use of mars to model healthcare
...
\ImportTok{import}\NormalTok{ mars }\ImportTok{as}\NormalTok{ earth}
...
The following example demonstrates the use of mars on New Zealand
...
\ImportTok{from}\NormalTok{ mars.explain }\ImportTok{import}\NormalTok{ plot\_partial\_dependence}
...
To demonstrate the practical application of mars with real health
...
The above examples demonstrate how mars can identify complex,
...
These capabilities make mars particularly valuable for health economic
...
Beyond the examples shown above, mars has broad applicability in
...
When applying mars to health economic data, several considerations are
important:
...
\item
  ...
  variables (e.g., ethnicity, diagnostic codes) that mars can handle
...
\item
  ...
  values. mars includes specialized functions for handling missing
...
One of the significant planned enhancements for mars is the addition
of a JAX/XLA backend option. This enhancement is critical for scaling
mars to larger healthcare datasets, which are increasingly common in
...
\tightlist
\item
  \textbf{API Compatibility}: The same mars interface regardless of
  backend
\item
...
implementation of mars provides accessibility benefits at the cost of
...
The mars library was developed following a systematic software
...
The MARS algorithm implementation in mars follows the original
...
The mars implementation was validated through multiple approaches to
...
\item
  \textbf{Regression Testing}: Results from mars were compared against
...
To ensure that mars produces statistically sound results, the
...
\item
  ...
  known analytical solutions, mars results were compared with
...
The mars implementation includes several optimizations to balance
...
mars provides a valuable pure Python implementation of the MARS
...
the effectiveness of mars in modeling complex relationships that
...
compatibility that makes mars valuable for researchers across diverse
...
The mars library addresses a specific need in the health economic
...
Python implementation with scikit-learn compatibility, mars lowers the
...
https://github.com/edithatogo/mars
