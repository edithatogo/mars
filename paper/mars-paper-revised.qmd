...
1. **Data Preprocessing**: Health data often includes categorical variables (e.g., ethnicity, diagnostic codes) that mars can handle directly, unlike some other algorithms that require one-hot encoding.

2. **Missing Data**: Health datasets frequently contain missing values. mars includes specialized functions for handling missing data, allowing for more complete analyses.
...
1. **GCV Evaluation**: For each basis function in the current model, evaluate the GCV score of the model with that function removed.

2. **Function Removal**: Remove the basis function that results in the lowest GCV score when removed.

3. **Iteration**: Repeat steps 2-3 until removing any remaining basis function would increase the GCV score.

4. **Coefficient Refitting**: The coefficients for the final selected model are refitted using least squares regression.
...
3. **Regression Testing**: Results from mars were compared against known test datasets to ensure consistency across versions.
...
2. **Comparison with Theoretical Results**: For simple cases with known analytical solutions, mars results were compared with theoretical expectations.
...
Mordaunt, D. A. (2025). mars: A Pure Python Implementation of Multivariate Adaptive Regression Splines. https://github.com/edithatogo/mars